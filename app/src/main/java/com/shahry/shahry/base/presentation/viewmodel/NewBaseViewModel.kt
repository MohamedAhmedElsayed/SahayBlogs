 ...
 */
abstract class NewBaseViewModel<STATE : ViewState, EVENT : ViewEvent, ACTION : Action, RESULT : Result>(
    /**
     ...
     */
    suspend fun onViewEvent(event: EVENT) {
        _uiEvent.send(event)
    }
     ...
     * @param showLoading [Boolean] the value indicating to show loading or not, default false
     */
    fun dispatch(
        action: ACTION,
